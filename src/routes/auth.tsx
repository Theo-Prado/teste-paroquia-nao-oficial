...
export const Route = createFileRoute("/auth")({
  head: () => ({
    meta: [
      { title: "Entrar — Paróquia Carlo Acutis" },
...
function AuthPage() {
  ...
  const { user, loading } = useAuth(); // 👈 IMPORTANTE (loading precisa existir no seu context)

  const [loadingForm, setLoadingForm] = useState(false);
  const [login, setLogin] = useState({ email: "", password: "" });
  const [signup, setSignup] = useState({ nome: "", email: "", password: "" });

  // 🔥 REDIRECT SEGURO (evita loop)
  useEffect(() => {
    if (loading) return; // espera auth carregar

    if (user) {
      navigate({
        to: isStaff(user) ? "/admin" : "/",
        replace: true,
      });
    }
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      emailSchema.parse(login.email);
      pwSchema.parse(login.password);
    } catch (err) {
      toast.error((err as z.ZodError).issues[0].message);
      return;
    }
    ...
    // ❌ não redireciona aqui — deixa o useEffect cuidar disso
  };
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    try {
      ...
      pwSchema.parse(signup.password);
    } catch (err) {
      toast.error((err as z.ZodError).issues[0].message);
      return;
    }
    ...
    <SiteLayout>
      <div className="mx-auto max-w-md px-4 py-16 sm:px-6">
        ...
        <p className="mt-1 text-center text-sm text-muted-foreground">
          Entre para administrar o site ou criar uma conta.
        </p>
          ...
          <TabsContent value="signup">
            ...
            >
              ...
              <Button
                ...
                disabled={loadingForm}
              >
                {loadingForm ? "Criando..." : "Criar conta"}
              </Button>

              <p className="text-xs text-center text-muted-foreground">
                O primeiro usuário a se cadastrar recebe acesso de administrador.
              </p>
            </form>
          </TabsContent>
        </Tabs>
